...
void TOutput::printf(const char* message, ...) {
    ...
    static const int STACK_BUF_SIZE = 1024;
...
#else
    std::string tmp_msg = "tid=";//----------------";
    tmp_msg += convert<std::string>(CurTid());
    tmp_msg += "  ";
    tmp_msg += message;
    //  tmp_msg += "\n";
...
//    printf(L_RED "usage:" YELLOW " input capacity\n" NONE);
    ...
    va_start(ap, tmp_msg.c_str()/*message*/);
    int need = vsnprintf(stack_buf, STACK_BUF_SIZE, tmp_msg.c_str()/*message*/, ap);
    va_end(ap);
