...
#define STRING_LIMIT_DEFAULT (1024 * 1024) // 1M
#define CONTAINER_LIMIT_DEFAULT (200* 10000) // YHW
/**
...
public:
  TBinaryProtocolT(boost::shared_ptr<Transport_> trans)
    : TVirtualProtocol<TBinaryProtocolT<Transport_, ByteOrder_> >(trans),
      trans_(trans.get()),
      /*string_limit_(0),*/
      string_limit_(STRING_LIMIT_DEFAULT),
      /*container_limit_(0),*/
      container_limit_(CONTAINER_LIMIT_DEFAULT),
      strict_read_(false),
      strict_write_(true) {}
  ...
  TBinaryProtocolT(boost::shared_ptr<Transport_> trans,
    ...
    : TVirtualProtocol<TBinaryProtocolT<Transport_, ByteOrder_> >(trans),
      ...
      strict_write_(strict_write) {
          ...
          if (string_limit_ == 0)       { string_limit_ = STRING_LIMIT_DEFAULT; }
          if (container_limit_ == 0)    { container_limit_ = CONTAINER_LIMIT_DEFAULT; }
      }

  void setStringSizeLimit(int32_t string_limit = STRING_LIMIT_DEFAULT) { string_limit_ = string_limit; }

  void setContainerSizeLimit(int32_t container_limit = CONTAINER_LIMIT_DEFAULT) { container_limit_ = container_limit; }
