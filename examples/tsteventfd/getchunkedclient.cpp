...
static void print_uri_parts_info(const struct evhttp_uri* http_uri)
{
    ...
    printf("------------------print_uri_parts_info end\n");
}
...
void http_requset_post_cb(struct evhttp_request* req, void* arg)
{
    ...
    switch (req->response_code)
    {
        case HTTP_OK:
        {
        printf( "http_requset_post_cb HTTP_OK\n" );
        ...
        case HTTP_MOVEPERM: {
                printf( "http_requset_post_cb HTTP_MOVEPERM\n" );
                printf("%s", "the uri moved permanently\n");
                break;
        }
        case HTTP_MOVETEMP:
        {
        printf( "http_requset_post_cb HTTP_MOVETEMP\n" );
        ...
        default: {
            printf( "http_requset_post_cb default\n" );
                event_base_loopexit(http_req_post->base, 0);
                return;
        }
    }
...
int start_url_request(void* http_req, int req_get_flag)
{
    ...
    {
        struct http_request_get* http_req_get = (struct http_request_get*) http_req;
...
void* start_http_requset(struct event_base* base, const char* url,
                         int req_get_flag, const char* content_type, const char* data)
{
    printf( "start_http_requset url=%s\n", url );
    void* http_req = http_request_new(base, url, req_get_flag, content_type, data);
    start_url_request(http_req, req_get_flag);
    return http_req;
}

int tst_getchunked_url()
{
...
//                                                          NULL, NULL);
    ...
    struct http_request_get* http_req_get =
        (struct http_request_get*) start_http_requset(base,
//                                                      "http://download.jetbrains.8686c.com/cpp/CLion-2016.1.3.tar.gz",
...
//    http_request_free(http_req_post, REQUEST_POST_FLAG);
    ...
    printf( "http_request_free(http_req_get -------------------\n" );
