...
调用成功时返回一个指向共享内存第一个字节的指针，如果调用失败返回-1.
     * */
    if((shm=(char*)shmat(shmid, (void*)nullptr, 0)) == (void *)-1)
    {
        perror("shmat");
        return NULL;
    }
...
static struct sq_head_t *open_shm_queue(long shm_key, long ele_size, long ele_count, int create)
{
    ...
    if(create)
    {
        ...
        printf("shm size needed for queue - %lu else_size=%lu ele_count=%lu\n",
               allocate_size, ele_size, ele_count);
    } else {
        allocate_size = 0;
    }
    ...
    {
        printf( "attach_shm shm=%0x and create!!!\n", shm );
...
struct sq_head_t *sq_create(u64_t shm_key, int ele_size, int ele_count)
{
    ...
    queue = open_shm_queue(shm_key, ele_size, ele_count, true);
    if(queue==NULL)
    {
...
{
    struct sq_head_t *queue = open_shm_queue(shm_key, 0, 0, false);
    if(queue==NULL)
    {
