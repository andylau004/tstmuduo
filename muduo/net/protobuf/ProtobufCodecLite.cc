...
void ProtobufCodecLite::fillEmptyBuffer(Buffer *buf, const google::protobuf::Message &message) {
    ...
    int checkSum = checksum(buf->peek(), static_cast<int>(buf->readableBytes()));
    buf->appendInt32(checkSum);

    assert(buf->readableBytes() == tag_.size() + byte_size + kChecksumLen); (void)byte_size;

    int len = sockets::hostToNetwork32(static_cast<int32_t>(buf->readableBytes()));
...
//    int byte_size = (int)message.ByteSize();
    int byte_size = (int)message.ByteSizeLong();
