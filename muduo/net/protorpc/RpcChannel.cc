...
RpcChannel::RpcChannel()
    : codec_(boost::bind(&RpcChannel::onRpcMessage, this, _1, _2, _3)),
      services_(NULL)
{
    LOG_INFO << "RpcChannel::cstor -" << this;
}
                              ...
                              Timestamp receiveTime)
{
    ...
    else if (message.type() == REQUEST)
    {
        ...
        if (services_)
        {
            /*std:map*/std::unordered_map<std::string, ::google::protobuf::Service*>::const_iterator it = services_->find(message.service());
            if (it != services_->end()) {
                ...
                if (method) {
                    ...
                    if ( request->ParseFromString(message.request()) ) {
                        ...
                        service->CallMethod(method,
                                            NULL,
                                            request.get()/*muduo::get_pointer(request)*/,
                                            response,
