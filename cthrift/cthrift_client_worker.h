...
//    map_ipport_sgservice_;  // use compare and update svrlist
    ...
    boost::shared_ptr<TMemoryBuffer>* sp_p_tmemorybuffer_;
    ...
    boost::unordered_map<std::string, SharedContSharedPtr> map_id_sharedcontextsp_;
...
//        CTHRIFT_LOG_DEBUG("into worker thread clean resource");
    ...
    virtual ~CthriftClientWorker() {
        ...
        p_event_loop_->runInLoop(
                    boost::bind(&CthriftClientWorker::ClearTcpClient, this, &clear_countdown));
    ...
    void DelContextMapByID(const std::string &str_id) {
        ...
        if (map_iter != map_id_sharedcontextsp_.end()) {
            ...
            if (sp_send_conn) {
//                CTHRIFT_LOG_WARN("del id from ipport :" << (sp_send_conn->peerAddress()).toIpPort());
            }
        }
