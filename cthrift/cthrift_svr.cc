...
__thread boost::shared_ptr<TMemoryBuffer>* CthriftSvr::sp_p_input_tmemorybuffer_;
__thread boost::shared_ptr<TMemoryBuffer>* CthriftSvr::sp_p_output_tmemorybuffer_;
...
void CthriftSvr::InitStaticThreadLocalMember(void) {
    ...
    if (CTHRIFT_LIKELY(!sp_p_input_tmemorybuffer_ || !(*sp_p_input_tmemorybuffer_))) {
        sp_p_input_tmemorybuffer_ = new boost::shared_ptr<TMemoryBuffer>(boost::make_shared<TMemoryBuffer>());
    }

    if (CTHRIFT_LIKELY(!sp_p_output_tmemorybuffer_ || !(*sp_p_output_tmemorybuffer_))) {
        sp_p_output_tmemorybuffer_ = new boost::shared_ptr<TMemoryBuffer>(boost::make_shared<TMemoryBuffer>());
    }
    ...
    if (CTHRIFT_LIKELY(!sp_p_processor_ || !(*sp_p_processor_))) {
      sp_p_processor_ =
          new boost::shared_ptr<TProcessor>(getProcessor(*sp_p_input_tprotocol_,
                                                         *sp_p_output_tprotocol_,
                                                         boost::make_shared<TNullTransport>()));
    }
...
int32_t CthriftSvr::Init(void) {
    ...
    i16_conn_thread_num_ = static_cast<int16_t >(g_cthrift_config.server_conn_threadnum_);
...
//    }
    ...
    try {
        sp_server_ = boost::make_shared<muduo::net::TcpServer>(&event_loop_,
...
//    }
    ...
    sp_server_->setConnectionCallback(
                boost::bind(&CthriftSvr::OnConn, this, _1));
    sp_server_->setMessageCallback(
                boost::bind(&CthriftSvr::OnMsg, this, _1, _2, _3));
    ...
    for (int i = 0; i < i16_worker_thread_num_; i ++) {
        ...
        EventLoopThread *p_eventloop_thread
                = new EventLoopThread(boost::bind(&CthriftSvr::WorkerThreadInit,
                                                  this,
    ...
    event_loop_.runEvery(WORKER_THREAD_POS_SECOND,
                         boost::bind(&CthriftSvr::InitWorkerThreadPos, this));
    return SUCCESS;
}
...
//    CTHRIFT_LOG_DEBUG("argument: " << str_argument);
    ...
    if (CTHRIFT_UNLIKELY(str_app_key.empty() || 0 == u16_port || (1 == i8_check_type && (0 > i32_svr_overtime_ms
...
void CthriftSvr::OnConn(const TcpConnectionPtr &conn) {
    ...
    if (conn->connected()) {
        ...
        sp_conn_info->wp_conn_entry = sp_conn_entry;
    ...
    while (more) {
        if (sp_conn_info->enum_state == kExpectFrameSize) {
            ...
            if (buffer->readableBytes() >= sizeof(int32_t)) {
                sp_conn_info->i32_want_size = static_cast<uint32_t>(buffer->readInt32());
            ...
            if (buffer->readableBytes() >= (size_t)(sp_conn_info->i32_want_size)) {

                atom_i64_recv_msg_per_min_.increment();// stat increment
