...
//            CTHRIFT_LOG_INFO("conn " << (sp_conn->peerAddress()).toIpPort() << " timeout");
            sp_conn->shutdown();
        }
    }
};
...
class CthriftSvr : boost::noncopyable, public TServer {
private:
    // time wheel
    typedef boost::shared_ptr<ConnEntry> ConnEntrySharedPtr;
    // more than one data entry in one grid in circule buffer
    typedef boost::unordered_set<ConnEntrySharedPtr> ConnEntryBucket;

    typedef boost::circular_buffer<ConnEntryBucket> ConnEntryBucketCirculBuf;
    typedef muduo::ThreadLocalSingleton <ConnEntryBucketCirculBuf> LocalSingConnEntryCirculBuf; // kick idle conn

    // static const double kDCheckConnIntervalSec;
    static const double kTMaxCliIdleTimeSec;
    static const int8_t kI8TimeWheelGridNum;

    static __thread boost::shared_ptr<TMemoryBuffer>* sp_p_input_tmemorybuffer_;
    static __thread boost::shared_ptr<TMemoryBuffer>* sp_p_output_tmemorybuffer_;

    static __thread boost::shared_ptr<TProtocol>* sp_p_input_tprotocol_;
    static __thread boost::shared_ptr<TProtocol>* sp_p_output_tprotocol_;
...
public:
    ...
    CthriftSvr(const boost::shared_ptr<Processor> &processor, THRIFT_OVERLOAD_IF(Processor, TProcessor)) throw(TException)
        : TServer(processor),
