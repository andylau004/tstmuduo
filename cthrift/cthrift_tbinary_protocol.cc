...
writeMessageBegin(const std::string &name,
    ...
    if (0 == seqid) {
        i32_seq_id = g_atomic_i32_seq_id.incrementAndGet();
...
//                              << i32_seq_id);
        ...
        boost::dynamic_pointer_cast<CthriftTransport>
                (getTransport())->SetID2Transport(str_id); //Transport to CthriftTransport, CANNOT
        // use dynamic_cast
    }
...
uint32_t CthriftTBinaryProtocolT<Transport_>::writeFieldStop() {
    return writeByte(static_cast<int8_t>(T_STOP));
}
...
//                          << " read size:" << size);
    ...
    if (size > this->string_buf_size_ || this->string_buf_ == NULL) {
        void *new_string_buf = std::realloc(this->string_buf_, static_cast<uint32_t>( size ));
